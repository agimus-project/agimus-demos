...
class HPPLeftArmSquareMotion:
    ...
    def plan_cartesian_trajectory(self, q0: list):
        ...
        self.wdhpp.cproblem = wd(
            self.ps.client.basic.problem.createProblem(self.wdhpp.crobot)
        )
        self.wdhpp.csm = wd(
        ...
        self.wdhpp.cs = wd(
            self.ps.client.basic.problem.createConstraintSet(
                self.wdhpp.crobot, "sm-constraints"
            )
        )
        self.wdhpp.cp = wd(
        ...
        self.first_way_point_in_world_se3 = pin.XYZQUATToSE3(
            self.robot.getJointPosition(self.ee_link_name)
        )
        self.base_in_world_se3 = pin.XYZQUATToSE3(
            self.robot.getJointPosition(self.base_link_name)
        )
        self.first_way_point_in_base_se3 = (
            self.base_in_world_se3.inverse() * self.first_way_point_in_world_se3
        )
        self.first_way_point_in_base = pin.SE3ToXYZQUAT(
            self.first_way_point_in_base_se3
        ).tolist()
        self.base_in_first_way_point_se3 = self.first_way_point_in_base_se3.inverse()
        self.base_in_first_way_point = pin.SE3ToXYZQUAT(
            self.base_in_first_way_point_se3
        ).tolist()
        ...
        derivative = [[0, 0]] * 6
        p1 = wd(
            self.ee_spline.createSplinePath(
                rhs0, rhs1, 1.0, [1, 2], derivative, [1, 2], derivative
            )
        )
        p2 = wd(
            self.ee_spline.createSplinePath(
                rhs1, rhs2, 1.0, [1, 2], derivative, [1, 2], derivative
            )
        )
        p3 = wd(
            self.ee_spline.createSplinePath(
                rhs2, rhs3, 1.0, [1, 2], derivative, [1, 2], derivative
            )
        )
        p4 = wd(
            self.ee_spline.createSplinePath(
                rhs3, rhs4, 1.0, [1, 2], derivative, [1, 2], derivative
            )
        )
        p5 = wd(
            self.ee_spline.createSplinePath(
                rhs4, rhs1, 1.0, [1, 2], derivative, [1, 2], derivative
            )
        )
    ...
    def plot_hpp_results(self, hpp_results: HPPResults):
        ...
        s1 = wd(
            spline2.createSplinePath(
                sq1, sq2, 1.0, [1, 2], derivative, [1, 2], derivative
            )
        )
        s2 = wd(
            spline2.createSplinePath(
                sq2, sq3, 1.0, [1, 2], derivative, [1, 2], derivative
            )
        )
        s3 = wd(
            spline2.createSplinePath(
                sq3, sq4, 1.0, [1, 2], derivative, [1, 2], derivative
            )
        )
        s4 = wd(
            spline2.createSplinePath(
                sq4, sq5, 1.0, [1, 2], derivative, [1, 2], derivative
            )
        )
        s = s1.asVector()
        s.appendPath(s2)
        ...
        for t in np.arange(0, duration, dt):
            ...
            data_plot.q = (
                np.vstack((data_plot.q, np.array(d.q)))
                if data_plot.q.size
                else np.array(d.q)
            )
            data_plot.v = (
                np.vstack((data_plot.v, np.array(d.v)))
                if data_plot.v.size
                else np.array(d.v)
            )
            data_plot.ee = (
