...
class MocapEEPublisher(Node):
    ...
    def __init__(self, mocap_ip: str, rate_hz: float):
        ...
        self.get_logger().info(
            "Mocap connected. Publishing mocap_ee at %.0f Hz." % rate_hz
        )
    ...
    def _publish(self) -> None:
        ...
        if np.any(np.isnan(positions[_BASE_IDX])) or np.any(
            np.isnan(positions[_EE_IDX])
        ):
            return

        T_base = pin.XYZQUATToSE3(
            np.concatenate([positions[_BASE_IDX], quats[_BASE_IDX]])
        )
        ...
        self._pose_pub.publish(_se3_to_pose_stamped(T_ee_in_base, stamp, _BASE_FRAME))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Publish mocap EE pose as TF + PoseStamped."
    )
